...
global $download_multi_sizes, $multi_download_var, $site_db;// MOD multi download
  ...
  if ($user_info['user_level'] != GUEST) {
    ...
    if (check_lightbox($image_row['image_id'])) {
      ...
      $lightbox_button = "<a href=\"".$site_sess->url($lightbox_url)."\"><img src=\"".get_gallery_image("lightbox_yes.png")."\" border=\"0\" alt=\"\" title=\"Tapeta znajduje się w Ulubionych\" /></a>";
    }
    else {
      $lightbox_url .= "action=addtolightbox&amp;id=".$image_row['image_id'];
      $lightbox_button = "<a href=\"".$site_sess->url($lightbox_url)."\"><img src=\"".get_gallery_image("lightbox_no.png")."\" border=\"0\" alt=\"\" title=\"Dodaj tapetę do Ulubionych\" /></a>";
    }
  }
  else {
    $lightbox_button = "<img src=\"".get_gallery_image("lightbox_off.png")."\" border=\"0\" alt=\"\" title=\"Zaloguj się i dodaj tapetę do Ulubionych\" />";
  }
  ...
  if (!check_permission("auth_sendpostcard", $image_row['cat_id'])) {
    $postcard_button = "<img src=\"".get_gallery_image("postcard_off.png")."\" border=\"0\" alt=\"\" title=\"Zaloguj się i wyslij tapetę jako e-kartkę\" />";
  }
  else {
    $postcard_button = "<a href=\"".$site_sess->url(ROOT_PATH."postcards.php?".URL_IMAGE_ID."=".$image_row['image_id'].((!empty($mode)) ? "&amp;mode=".$mode : ""))."\"><img src=\"".get_gallery_image("postcard.png")."\" border=\"0\" width=\"64\" height=\"64\" alt=\"\" title=\"Wyslij tapetę jako e-kartkę\" /></a>";
  }
  ...
  $site_template->register_vars(array(
    ...
    "cat_name" => isset($image_row['cat_name']) ? format_text($image_row['cat_name'], 2) : '',
    "cat_url" => (isset($image_row['cat_name']) && $image_row['cat_name']) ? get_category_seo_url($image_row['cat_id'], $image_row['cat_name']) : $site_sess->url(ROOT_PATH."categories.php?".URL_CAT_ID."=".$image_row['cat_id']),
	...
	"image_url" => ($show_link) ? get_image_seo_url($image_row['image_id'], $image_row['image_name'], $mode) : "",
...
// ######## Start Mod Additional Category #################    
  if (!empty($additional_category_fields)) {
    $additional_field_array = array();
    foreach ($additional_category_fields as $key => $val) {
...
$multi_download = isset($multi_download) ? $multi_download : '';
if ($multi_download && $detailed_view){
	...
	if (!check_permission("auth_download", $image_row['cat_id'])) {
		...
		$multi_download_resolution = "<TR><TD colspan='".$multi_download_var['columns']."'><p class='multi_resolution1'>".$lang['multi_resolution1']."<script type='text/javascript'>document.write(screen.width+'X'+screen.height)</script></p></TD></TR>";
		...
		if (!$multi_download_var['type']){
			$multi_download_options = "<BR><BR><form action='".$site_sess->url(ROOT_PATH."download.php")."' method='get'".$multi_download_var['target']."><table border='0' cellspacing='1' cellpadding='0'><tr><td><table border='0' cellspacing='0' cellpadding='0'>";
			if ($multi_download_var['show_resolution']){
				$multi_download_options .= $multi_download_resolution;
			}
			if ($multi_download_var['download']){
				$multi_download_options .="<TR><TD colspan='".$multi_download_var['columns']."' align='center'>".$lang['multi_select']."</TD></TR>";
			}
			$multi_download_options .="<TR><TD><select name='multi_download_select'>";
		}
		else{
			$multi_download_options = "<table border='0' cellspacing='1' cellpadding='0'><tr><td><table border='0' cellspacing='0' cellpadding='0'>";
			if ($multi_download_var['show_resolution']){
				$multi_download_options .= $multi_download_resolution;
			}
			if ($multi_download_var['download']){
				$multi_download_options .= "<tr><TD colspan='".$multi_download_var['columns']."' align='center'>".$lang['multi_select']."</TD>";
			}
		}
		$var=0;
		$down_group=0;
...
function get_thumbnail_code($media_file_name, $thumb_file_name = "", $image_id, $cat_id, $image_name = "", $mode = "", $show_link = 1, $open_window = 0) {
  ...
  if (!check_media_type($media_file_name)) {
    ...
    if (!get_file_path($thumb_file_name, "thumb", $cat_id, 0, 0)) {
      ...
      $image_info = @getimagesize($file_src);
      $width_height = (!empty($image_info[3])) ? " ".$image_info[3] : "";
      $thumb = "<img class=\"miniaturka\" width=\"220\" height=\"160\" src=\"".$file_src."\"  alt=\"".format_text($image_name, 2)."\" title=\"".format_text($image_name, 2)."\" />";
    }
  }
...
// SEO Friendly URL
  if ($show_link) {
    $seo_url = get_image_seo_url($image_id, $image_name, $mode);
    if ($open_window) {
      $thumb = "<a href=\"".$seo_url."\" onclick=\"opendetailwindow()\" target=\"detailwindow\">".$thumb."</a>";
    }
    else {
      $thumb = "<a href=\"".$seo_url."\">".$thumb."</a>";
    }
  }
  ...
  if (!check_media_type($media_file_name)) {
    ...
    if (!get_file_path($media_file_name, "thumb", $cat_id, 0, 0)) {
      ...
      $image_info = @getimagesize($file_src);
       ...
       $thumb = "<img src=\"".$file_src."\" style=\"border:0px;width:$thumb_width; height:$thumb_height;\" alt=\"".format_text($image_name, 2)."\" title=\"".format_text($image_name, 2)."\" />";
    }
    else {
      ...
      $thumb = "<img src=\"".$file_src."\" style=\"border:".$config['image_border']."px solid;\" alt=\"".format_text($image_name, 2)."\" title=\"".format_text($image_name, 2)."\" />";
    }
  }
...
function get_media_code($media_file_name, $image_id = 0, $cat_id = 0, $image_name = "", $mode = "", $show_link = 0, $detailed_view = 0, $big = 0) {
  ...
  if (!get_file_path($media_file_name, "media", $cat_id, 0, 0)) {
    ...
    $media_src = get_file_path($media_file_name, "big", $cat_id, 0, 1);
...
function replace_url($text) {
  ...
  $url_replace_array = array(
    "\\1<a href=\"https://www.\\2.\\3\\4\" target=\"_blank\" rel=\"nofollow\">www.\\2.\\3\\4</a>",
...
function format_text($text, $html = 0, $word_wrap = 0, $bbcode = 0, $bbcode_img = 0) {
  ...
  if ($bbcode == 1) {
    ...
    $replace_array = array(
      ...
      "<li>",
      "</li>",
      "<a href=\"https://www.\\6\" target=\"_blank\" rel=\"nofollow\">\\8</a>",
      "<a href=\"\\5\" target=\"_blank\" rel=\"nofollow\">\\7</a>",
      "<a href=\"https://www.\\5\" target=\"_blank\" rel=\"nofollow\">www.\\5</a>",
...
function get_categories($cat_id = 0) {
  ...
  $categories = "\n<div id=\"catthumb_resp\" width=\"".$config['cat_table_width']."\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\">\n";

  $count = 0;
  $count2 = 0;
  foreach ($visible_cat_cache as $key => $category_id) {
    $categories .= "<div class=\"catthumb_resp_box\" align=\"center\"><div class=\"catthumb_resp_content\">\n";
...
// ######## End Mod Additional Category ###################
    ...
    $site_template->register_vars(array(
      ...
      "cat_url" => get_category_seo_url($category_id, $cat_cache[$category_id]['cat_name']),
...
function get_category_path($cat_id = 0, $detail_path = 0) {
  ...
  while ($parent_id) {
    ...
    if (empty($path)) {
      if ($detail_path) {
        $cat_url = get_category_seo_url($cat_id, $cat_cache[$cat_id]['cat_name']);
        if (preg_match("/".URL_PAGE."=([0-9]+)/", $url, $regs)) {
          if (!empty($regs[1]) && $regs[1] != 1) {
            $cat_url .= (strpos($cat_url, '?') !== false ? "&amp;" : "?") . URL_PAGE."=".$regs[1];
          }
        }
        $path = "<li><a href=\"".$cat_url."\">".format_text($cat_cache[$cat_id]['cat_name'], 2)."</a></li>";
      }
      else  {
        $path = "<li><a href=\"".get_category_seo_url($cat_id, $cat_cache[$cat_id]['cat_name'])."\">".format_text($cat_cache[$cat_id]['cat_name'], 2)."</a></li>";
      }
    }
    else {
      $path = "<li><a href=\"".get_category_seo_url($cat_id, $cat_cache[$cat_id]['cat_name'])."\">".format_text($cat_cache[$cat_id]['cat_name'], 2)."</a></li>".$path;
    }
    $cat_id = $parent_id;
  } // end while
  return $path;
...
function get_category_dropdown($cat_id, $jump = 0, $admin = 0, $i = 0) {
  ...
  default:
    if ($jump) {
      $category = "\n<select name=\"".URL_CAT_ID."\" onchange=\"if (this.options[this.selectedIndex].value != 0){ forms['jumpbox'].submit() }\" class=\"\">\n";
    }
    else {
      $category = "\n<div class=\"tx-custom-select\" style=\"width:260px;\"><select name=\"".URL_CAT_ID."\" class=\"\">\n";
    }
    $category .= "<option value=\"0\">".$lang['select_category']."</option>\n";
    $category .= "<option value=\"0\">-------------------------------</option>\n";
  } // end switch
...
function get_thumbnail_rss_code($media_file_name, $thumb_file_name = "", $image_id, $cat_id, $image_name = "", $mode = "", $show_link = 1, $open_window = 1) {
  global $site_sess, $config;
  $mysite = 'https://www.tapeteos.pl/';// change this to the path to your 4images gallery
