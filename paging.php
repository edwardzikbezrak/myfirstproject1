...
// Usuñ istniej¹ce parametry page
$link_args = preg_replace("/&page=[0-9]*/", "", $link_args);
$link_args = preg_replace("/page=[0-9]*&/", "", $link_args);
$link_args = preg_replace("/\?page=[0-9]*/", "", $link_args);

// SprawdŸ czy to SEO URL (bez .php)
if (strpos($link_args, '.php') === false && strpos($link_args, 'http') === false) {
  // To jest SEO URL (np. "szukaj/panoramiczne")
  $this->link_args = $link_args;
  $this->link_args .= (strpos($this->link_args, '?') !== false) ? "&amp;" : "?";
} else {
  // Stary URL
