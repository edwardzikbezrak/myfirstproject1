...
function get_user_ip() {
    // Najpierw sprawdŸ Cloudflare
    if (isset($_SERVER['HTTP_CF_CONNECTING_IP'])) {
        return $_SERVER['HTTP_CF_CONNECTING_IP'];
    }
    // Nastêpnie sprawdŸ typowe nag³ówki proxy
    if (!empty($_SERVER['HTTP_CLIENT_IP']) && filter_var($_SERVER['HTTP_CLIENT_IP'], FILTER_VALIDATE_IP)) {
        return $_SERVER['HTTP_CLIENT_IP'];
    }
    if (!empty($_SERVER['HTTP_X_FORWARDED_FOR'])) {
        // W X-Forwarded-For mo¿e byæ lista IP, bierzemy pierwszy prawid³owy
...
$site_template->register_vars(array(
      "has_imageid"   => true,
      "domain" =>"<link rel=\"canonical\" href=\"".$script_url."/"."details.php?".URL_IMAGE_ID."=".$image_id.((!empty($mode)) ? "&mode=".$mode : "").(($page > 1) ? "&page=".$page :"\" />"
        )));
}
...
$site_template->register_vars(array(
      "has_catid"   => true,
      "domain" =>"<link rel=\"canonical\" href=\"".$script_url."/"."categories.php?".URL_CAT_ID."=".$cat_id."\" />"
        ));
}
